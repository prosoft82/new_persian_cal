...
import { JalaliDate, CalendarDay, Goal } from '../types';
...
export default function CalendarGrid({
  ...
  // Calculate grid days memoized for year
  const daysGrid: CalendarDay[] = useMemo(() => {
    return getMonthDays(activeYear, activeMonth);
    ...
    <div id="calendar-grid-container" className="bg-white border border-stone-200 p-6 shadow-sm select-none rounded-xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-stone-100 pb-5 mb-5">
        ...
        <div className="flex items-center justify-between md:justify-start w-full md:w-auto gap-4">
          ...
          <div className="text-right">
            <span className="text-[10px] uppercase tracking-wider text-stone-400 font-bold block mb-0.5">تقویم سال {activeYear}</span>
      ...
      <div className="grid grid-cols-7 gap-2">
        {daysGrid.map((day, ix) => {
          ...
          // Find which goals have been crossed on this day
          const crossedGoalsCount = goals.filter(g => (g.crossedDays || []).includes(dateKey)).length;
          const isCrossed = crossedGoalsCount > 0;
            ...
            >
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                  <div className="relative w-full h-full flex items-center justify-center">
                    ...
                    <span className={`text-[4.5rem] sm:text-[5.5rem] font-bold select-none leading-none opacity-80 ${
                      isSelected ? 'text-rose-400/80' : 'text-rose-600/70'
                    }`}>
                      ×
                    </span>
                  </div>
              ...
              <div className="flex justify-between items-start z-20">
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  {/* Small colored dots for completed goals */}
                  {crossedGoalsCount > 0 && (
                    <div className="flex gap-0.5 items-center mt-0.5">
                      {goals.map((g, gi) => {
                        const met = (g.crossedDays || []).includes(dateKey);
                        if (!met) return null;
                        return (
                          <span
                            key={g.id}
                            className={`w-1.5 h-1.5 rounded-full ${
                              gi === 0 ? 'bg-amber-500' : gi === 1 ? 'bg-emerald-500' : 'bg-blue-400'
                            }`}
                            title={`هدف شماره ${gi + 1} حاصل شد`}
                          />
                        );
                      })}
                    </div>
      ...
      {/* Legend / Info Footer */}
      <div className="mt-5 pt-4 border-t border-stone-100 flex flex-wrap items-center gap-x-5 gap-y-2.5 text-xs text-stone-500 justify-start select-none">
        <div className="flex items-center gap-1.5">
          <span className="w-3.5 h-3.5 bg-emerald-50 border border-emerald-300 rounded block"></span>
          <span>امروز</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-stone-900 block"></span>
        ...
        {goals.length > 0 && (
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-1.5 text-stone-700">
              <span className="text-rose-600 font-black font-sans text-sm">×</span>
              <span>روز متعهد و خط زده شده (❌)</span>
            </div>
            
            {/* Color key for goals mapping */}
            <div className="flex items-center gap-2 border-r border-stone-200 pr-3 mr-1">
              <span className="text-[10px] text-stone-400">راهنمای اهداف خلاصه:</span>
              {goals.map((g, gi) => (
                <div key={g.id} className="flex items-center gap-1">
                  <span className={`w-2 h-2 rounded-full ${
                    gi === 0 ? 'bg-amber-500' : gi === 1 ? 'bg-emerald-500' : 'bg-blue-400'
                  }`} />
                  <span className="text-[10px] text-stone-600">هدف {gi + 1}</span>
                </div>
              ))}
            </div>
          </div>
